...
{   fn build( &self, app: &mut App )
    {   app
        ...
        .add_systems
        (   Update,
            (   is_loading_done, //ロード完了ならState変更
                move_sprite,     //ロード中のアニメ
            )
...
(   assets: Res<LoadedAssets>,
    o_after_load_assets: Option<Res<AfterLoadAssets<MyState>>>,
...
{   //事前ロードが完了したか？
    ...
    //次のStateへ遷移する
    let Some ( after_load_assets ) = o_after_load_assets else { return };
    next_state.set( after_load_assets.state );
}
...
{   let mut rng = rand::thread_rng();
    ...
    {   for ( goal_x, char ) in line.chars().enumerate()
        {   //空白文字は無視
            ...
            let start = IVec2::new( rnd_x, rnd_y ).convert_pixel();
...
{   let time_delta = time.delta().as_secs_f32() * 5.0;
    ...
    let scale = SCREEN_PIXELS_WIDTH / NOWLOADING.width; //横方向に長いので
    ...
    (   | ( mut transform, goal ) |
        {   //座標の調整
            let mut px2d = goal.grid.convert_pixel();
